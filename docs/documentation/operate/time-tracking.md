...
## How does time tracking work in Linesbox

Like everything we do in Linesbox, we try to keep features as simple as possible. Time tracking is no different - it’s simple to understand, simple to use and works as you think it works.
...
All employees in Linesbox can use the time tracking feature, as it’s enabled by default on every company’s account.
...
Like with every other features in Linesbox, privacy has been taken into account when designing this feature. No regular employees can see someone else’s timesheets. Only employees with a specific right can read them:
...
While an employee creates an entry on his dashboard, time tracking entries appear on several places in Linesbox:
...
There are several ways to log time in Linesbox:
...
* a duration: the duration is written in hours and minutes. While Linesbox will make sure that this duration doesn't exceed 24 hours, it won't limit employees otherwise.
...
Linesbox will make sure that you can not enter more than 24 hours for all entries in a given day.
...
When there are no more timesheets to approve, Linesbox will  show a nice empty state.
